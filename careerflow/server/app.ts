...
import { JOB_STATUSES, type JobInput } from "./types.js";
...
function validateJob(body: Partial<JobInput>) {
...
export function createApp(databasePath: string) {
  ...
  app.post("/api/auth/login", (request, response) => {
    ...
    response.setHeader(
      "Set-Cookie",
      `careerflow_session=${result.token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=604800`
    );
  ...
  app.post(
    ...
    (request: AuthenticatedRequest, response) => {
      ...
      response.setHeader(
        "Set-Cookie",
        "careerflow_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0"
      );
      response.status(204).end();
    }
  );

  app.get("/api/jobs", requireAuth, (request: AuthenticatedRequest, response) => {
    response.json({ jobs: db.listJobs(request.user!.id) });
  });

  app.post("/api/jobs", requireAuth, (request: AuthenticatedRequest, response) => {
  ...
  app.post("/api/analyze", requireAuth, (request, response) => {
    const { resumeText, jobDescription } = request.body as {
      resumeText?: string;
